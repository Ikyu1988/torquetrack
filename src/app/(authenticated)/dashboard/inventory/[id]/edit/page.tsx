...
export default function EditPartPage() {
  ...
  useEffect(() => {
    if (partId && isMounted) {
      setIsLoading(true);
      let partData: Part | undefined;
      if (typeof window !== 'undefined' && (window as any).__inventoryStore) {
        partData = (window as any).__inventoryStore.getPartById(partId);
      }
      ...
      if (partData) {
        form.reset({
          ...
          cost: partData.cost === undefined ? '' : partData.cost,
          supplier: partData.supplier || "",
          stockQuantity: partData.stockQuantity,
          minStockAlert: partData.minStockAlert === undefined ? '' : partData.minStockAlert,
  ...
  }, [partId, form, router, toast, isMounted]);

  function onSubmit(data: PartFormValues) {
    let success = false;
    if (typeof window !== 'undefined' && (window as any).__inventoryStore) {
      const existingPart = (window as any).__inventoryStore.getPartById(partId);
      if (existingPart) {
        ...
        success = (window as any).__inventoryStore.updatePart(updatedPartData);
      }
    }
    ...
    if (success) {
      toast({
        title: "Part Updated",
        description: `Part "${data.name}" has been successfully updated.`,
      });
      router.push("/dashboard/inventory");
    } else {
       toast({
  ...
  if (!isMounted || isLoading) {
    const message = !isMounted ? "Loading form..." : "Loading part data...";
      ...
      <Card className="shadow-lg">
        <CardHeader>
          <div className="flex items-center gap-3">
            ...
            <CardTitle className="font-headline text-2xl">Edit Part</CardTitle>
          </div>
          <CardDescription>Update the details for this inventory part.</CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
