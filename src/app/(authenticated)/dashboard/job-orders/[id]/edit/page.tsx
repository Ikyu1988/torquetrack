...
export default function EditJobOrderPage() {
  ...
  useEffect(() => {
    setIsMounted(true);
    // Load customers and motorcycles for dropdowns
  ...
  useEffect(() => {
    if (jobOrderId && customers.length > 0 && motorcycles.length > 0 && isMounted) {
      setIsLoading(true);
      ...
      if (jobOrderData) {
        form.reset({
          ...
          totalLaborCost: jobOrderData.totalLaborCost,
          totalPartsCost: jobOrderData.totalPartsCost,
  ...
  }, [jobOrderId, form, router, toast, customers, motorcycles, isMounted]);
  
  const filteredMotorcycles = useMemo(() => {
    if (!selectedCustomerId) return motorcycles; // Show all if no customer selected (e.g. during initial load)
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea placeholder="List services performed..." {...field} rows={3}/>
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea placeholder="List parts used..." {...field} rows={3}/>
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
